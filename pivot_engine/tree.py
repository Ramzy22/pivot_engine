...
class TreeExpansionManager:
    ...
    def _build_level(
        ...
        cursor = path_cursor_map.get(path_key)

        level_spec = self._create_level_spec(base_spec, parent_path, current_dimension, cursor)
        
        result_dict = self.controller.run_pivot(level_spec, return_format="dict")
        nodes = self._results_to_dict(result_dict["rows"], result_dict["columns"])
        ...
        if result_dict.get("next_cursor"):
            ...
            nodes.append({
                "_is_load_more_node": True,
                "next_cursor": result_dict["next_cursor"],
                "parent_path_key": path_key,
            })

        for node in nodes:
            if node.get("_is_load_more_node"):
                continue
            
            node_path = parent_path + [node[current_dimension]]
            if state.is_expanded(node_path):
                node["children"] = self._build_level(
                    base_spec, spec_hash, state, node_path, dimension_hierarchy, level + 1, path_cursor_map
                )
        return nodes
    ...
    def _results_to_dict(
        ...
        for row_data in rows:
            node = {col: row_data[idx] for idx, col in enumerate(columns)}
            nodes.append(node)
    ...
    def _build_level_progressive(
        ...
        for node in nodes:
            if node.get("_is_load_more_node"):
                ...
                if state.is_expanded(node_path):
                    children = []
                    node["children"] = self._build_level_progressive(
                        base_spec, spec_hash, state, node_path, dimension_hierarchy,
                        level + 1, path_cursor_map, progress_callback, children
                    )
    ...
    def _build_level_chunked(
        ...
        for node in nodes:
            ...
            node_path = parent_path + [node[current_dimension]]
            if state.is_expanded(node_path):
                node["children"] = self._build_level_chunked(
                    base_spec, spec_hash, state, node_path, dimension_hierarchy,
                    level + 1, path_cursor_map, chunk_size
                )
        return nodes
        ...
        for node in nodes:
            ...
            if state.is_expanded(node_path):
                ...
                if prefetch_depth > 0:
                    ...
                    node["children"] = self._build_level_with_prefetch(
                        base_spec, spec_hash, state, node_path, dimension_hierarchy,
                        level + 1, path_cursor_map, prefetch_depth - 1
                    )
                else:
                    # Just mark that children exist without loading them
                    node["children"] = []  # Will be loaded on demand
                    node["_has_children"] = True
            elif level < len(dimension_hierarchy) - 1 and prefetch_depth > 0:
                # Check if this node would have expanded children in the full tree
                # For now, we'll add an indicator for UI to know
    ...
    def _load_multiple_levels_batch(
        ...
        for parent_path, paths in path_groups.items():
            ...
            path_key = self._expansion_state_to_key(list(parent_path))
    ...
    def _expansion_state_to_key(self, path: List[str]) -> str:
        """Convert path to string key, using the same logic as ExpansionState.path_to_key"""
        return "|".join(str(v) for v in path)